...
FishBody.pointsArray = [];
FishBody.normalsArray = [];

FishBody.getVertex = function (u, v)
{
    var radius = 1.0 - v ;
    var vd = {} ;
    vd.position = vec4(radius*Math.cos(u*2*Math.PI), radius*Math.sin(u*2*Math.PI), v-0.5, 1.0) ; // last variable is the scaling factor of the entire object
...
FishBody.init = function(n, program)
{
    
    this.n = n ; // Number of edges fo the base polygon
    if( this.n < 1) return;
...
Sphere.init = function(n, program)
{
    ...
    var startIndex = this.totalVertices;
    this.totalVertices += (this.n * this.n * 6);
